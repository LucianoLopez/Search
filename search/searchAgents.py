...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # problem.updateState(state[0])
    dist = 0
    game = problem.gameState
    pointA = state[0]
    pointB = None
    for corner in corners:
        if corner not in state[1]:
            pointB = corner
            distfromMaze = mazeDistance(pointA, pointB, game) 
            if dist < distfromMaze:
                dist = distfromMaze
            # pointA = corner
    return max(dist, 4-len(state[1]))
...
def foodHeuristic(state, problem):
    ...
    while len(foodlist) != 0:
        pointB = foodlist.pop()
        distFromMaze = mazeDistance(position, pointB, gameState)
        if dist < distFromMaze:
            dist = distFromMaze
    return dist
    ...
    # return 0
    # dist = 0  This is from CornersHeuristic, using it as reference
    # game = problem.gameState
    # pointA = state[0]
    # pointB = None
    # for corner in corners:
